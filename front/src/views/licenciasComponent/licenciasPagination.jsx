...
import { getLicencias, getTotalOfLicencias,  } from '../../redux/actions';
import { useEffect, useState } from "react";

function LicenciasPaginationComponent() {
  const [totalPages, setTotalPages] = useState(1);

  const dispatch = useDispatch();
  const filter = useSelector((state) => state.selectedFilterLicencia);
  const currentPagina = useSelector((state) => state.currentPageLicencia);

  useEffect(() => {
    const fetchTotalPages = async () => {
      const totalItems = await dispatch(getTotalOfLicencias(filter));
  ...
  const handlePage = (page) => {
    dispatch(getLicencias(page, filter));
  };
  ...
  const renderPageNumbers = () => {
    ...
    return pages.map((page, index) => (
      <button
        ...
        className={`px-4 py-2 text-sm font-semibold border border-[#0477AD] ${
          page === currentPagina
...
export default LicenciasPaginationComponent;
